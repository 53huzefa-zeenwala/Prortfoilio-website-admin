...
function ListedImageInput({ values, setValues, name, desc }) {
    ...
    const onChangeHandler = () => {
        const newValue = [currentImage, ...values[name]]
        setValues(prevState => ({
            ...prevState,
            [name]: newValue
        }))
    ...
    const ImageList = [
        { index: 0, image: "/boy-512.webp", name: "userImage.png" },
        { index: 1, image: "/newproject.png", name: "projectImage.png" },
    ]
    return (
        <div className='w-full'>
            ...
            {ImageList && <ul className='space-y-2 py-2'>
                {ImageList.map((val, i) => (
                    <li key={i} className="flex w-full items-center border border-gray-700 bg-gray-600 px-2 rounded shadow-lg shadow-slate-900/50">
                        ...
                        <span className='w-full flex items-center gap-2 text-sm text-gray-50 rounded font-medium cursor-not-allowed h-20 '>
                            <span className='h-full relative aspect-square border border-gray-500 bg-gray-100'>
                                <Image src={val.image} alt={val.name} fill="object-fit" />
                            </span>
                            <span>{val.name}</span>
                        </span>
                        <svg className='pl-4 pr-2 h-7 hover:fill-gray-400 fill-gray-300 transition-colors cursor-pointer' viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
