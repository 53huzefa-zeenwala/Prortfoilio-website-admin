...
export default function PrimaryButton({ text, color, isLoading, isSuccess, setIsSuccess }) {
    useEffect(() => {
        if (isSuccess === true) {
    ...
    return (
        <button disabled={isLoading || isSuccess} className={`${isSuccess ? "bg-green-200" : "bg-blue-600 hover:bg-blue-500"} px-6 py-2 rounded flex justify-center items-center transition-colors disabled:cursor-not-allowed shadow-lg shadow-slate-900/50`} type="submit">
            ...
            {isLoading && (
                <div role="status">
                    <svg className="mr-2 w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 dark:fill-gray-100 fill-gray-300" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {isSuccess && (<div style={{visibility: isSuccess ? "visible" : "hidden"}} className="w-8 h-8 rounded-full flex justify-center items-center border-2 border-green-500 transition-[visibility]">
