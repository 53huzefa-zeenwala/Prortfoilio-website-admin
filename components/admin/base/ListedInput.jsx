...
function ListedInput({ type = "text", maxLen = 20, minLen = 5, placeholder, values, setValues, name, desc }) {
    ...
    const onChangeHandler = () => {
        const newValue = [currentValue, ...values[name]]
        setValues(prevState => ({
            ...prevState,
            [name]: newValue
        }))
    ...
    return (
        <div className='w-full'>
            <div className="flex space-x-3">
                <input type={type} name={name} minLength={minLen} maxLength={maxLen} placeholder={placeholder} value={currentValue} onChange={e => setCurrentValue(e.target.value)} onKeyDown={e => e.key == "Enter" && onChangeHandler} className="px-4 sm:py-3 py-2.5 bg-gray-200 text-gray-800 placeholder:text-gray-500 border border-gray-500 outline-none hover:bg-gray-100 hover:placeholder:text-gray-400 focus:placeholder:text-gray-400 focus:bg-gray-50 focus:border-gray-400 rounded w-full text-sm" />
            ...
            {values[name] && <ul className='space-y-2 py-2'>
                {values[name].map((val, i) => (
                    <li key={i} className="flex w-full items-center border border-gray-700 bg-gray-600 py-2 px-2 rounded shadow-lg shadow-slate-900/50">
